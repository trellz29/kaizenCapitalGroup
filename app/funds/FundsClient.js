      ...
      <main className="min-h-screen bg-[#E6EEF2] text-[#0F1A28]">
        ...
        <section className="relative overflow-hidden px-4 pb-16 pt-32 sm:px-6 sm:pt-40">
          ...
          <div className="relative z-10 mx-auto max-w-6xl">
            ...
            <h1 className="max-w-4xl text-5xl font-bold leading-tight text-[#0F1A28] sm:text-6xl md:text-7xl" style={{ animation: "fadeUp 0.7s ease-out 0.2s both" }}>15 funds. One<br />institutional platform.</h1>
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-4" style={{ animation: "fadeUp 0.7s ease-out 0.45s both" }}>
              {[{label:"Total Funds",value:"15"},{label:"Live Funds",value:"6"},{label:"Brokerages",value:"3"},{label:"Avg Return",value:"9.2%"}].map(s=>(
            ...
            <div className="mb-10 flex flex-wrap gap-2">
              {filters.map(f=>(
                <button key={f} onClick={()=>setFilter(f)} className={`rounded-full px-4 py-2 text-sm font-semibold transition hover:scale-105 ${filter===f?"bg-[#0F1A28] text-white":"border border-[#C9D8E2] bg-white/60 text-[#2E4358] hover:bg-white/80"}`}>
                  {f} ({f==="All"?FUND_DATA.filter(d=>["Live","N/A","Re-Launching","Disconnected","Discontinuation"].includes(d.status)).length:FUND_DATA.filter(d=>d.status===f).length})
                </button>
              ))}
            </div>
            {!mounted ? (
