...
function useInView(threshold = 0.1) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) { setInView(true); observer.unobserve(node); } },
      { threshold }
    );
...
function FadeInSection({ children, className = "", id = "" }) {
  ...
  return (
    <section id={id || undefined} ref={ref}
...
/* ─── Sparkline SVG ──────────────────────────────────────────────────── */
function Sparkline({ data, color = "#1F5E36", height = 40, positive = true }) {
...
function MonthlyBarChart({ data }) {
  ...
  return (
    <div className="flex items-end gap-0.5 h-10">
      {data.map((d, i) => {
        const h = Math.max((Math.abs(d.val) / max) * 100, 8);
        const pos = d.val >= 0;
        return (
          <div key={i} className="flex-1 flex flex-col items-center justify-end" title={`${d.month}: ${d.val > 0 ? "+" : ""}${d.val}%`}>
            <div className="w-full rounded-sm transition-all duration-300"
              style={{ height: `${h}%`, background: pos ? "rgba(31,94,54,0.7)" : "rgba(122,47,47,0.6)" }} />
          </div>
        );
      })}
    </div>
...
function RiskMeter({ level }) {
  ...
  const bgColors = { Low: "bg-[#DCEFE3] text-[#1F5E36] border-[#B8D8C4]", Medium: "bg-[#F5EDD8] text-[#7A5C1E] border-[#E8D5AA]", High: "bg-[#F3E4E4] text-[#7A2F2F] border-[#E5C6C6]" };
  return (
    <div className="flex items-center gap-2">
      <div className="flex gap-0.5">
        {[1, 2, 3].map(i => (
          <div key={i} className="h-2.5 w-2 rounded-sm transition-all"
            style={{ background: i <= n ? colors[level] : "#D4E3EC" }} />
        ))}
      </div>
      <span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${bgColors[level]}`}>{level}</span>
    </div>
  );
}

/* ─── Enhanced Fund Card ─────────────────────────────────────────────── */
function FundCard({ label, name, focus, strategy, managers, brokerage, status, extra,
  primaryLink, secondaryLinks = [],
  totalReturn, winRate, maxDrawdown, sharpe, monthlyReturns, sparkData, riskLevel, minInvestment }) {
  ...
  const statusClass =
    isLive ? "bg-[#DCEFE3] text-[#1F5E36] border border-[#B8D8C4]" :
  ...
  return (
    <div className="group relative overflow-hidden rounded-3xl border border-white/60 bg-white/75 shadow-[0_12px_40px_rgba(15,26,40,0.06)] backdrop-blur-md transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_24px_70px_rgba(15,26,40,0.12)]">
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between mb-4">
          <div className="min-w-0">
            <p className="mb-1 text-[11px] font-semibold uppercase tracking-[0.15em] text-[#5A7188]">{label}</p>
            <h3 className="break-words text-lg font-bold leading-tight text-[#0F1A28] sm:text-xl">{name}</h3>
            <p className="mt-0.5 text-xs text-[#5A7188]">{focus}</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {isLive && <span className="h-1.5 w-1.5 rounded-full bg-[#1F5E36] animate-pulse" />}
            <span className={`rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${statusClass}`}>{status}</span>
          </div>
        </div>

        {/* Sparkline for live funds */}
        {sparkData && (
          <div className="mb-4 overflow-hidden rounded-2xl bg-[#F7FAFC]/60 px-3 pt-2 pb-1">
            <Sparkline data={sparkData} color="#2E4358" height={44} />
          </div>
        )}

        {/* Key metrics grid for live funds */}
        {hasMetrics && (
          <div className="mb-4 grid grid-cols-2 gap-2">
            {totalReturn && (
              <div className="rounded-2xl bg-[#F7FAFC]/80 px-3 py-2.5">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-[#5A7188]">Total Return</p>
                <p className={`mt-0.5 text-lg font-bold ${totalReturn.startsWith("+") ? "text-[#1F5E36]" : "text-[#7A2F2F]"}`}>{totalReturn}</p>
              </div>
            )}
            {winRate && (
              <div className="rounded-2xl bg-[#F7FAFC]/80 px-3 py-2.5">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-[#5A7188]">Win Rate</p>
                <p className="mt-0.5 text-lg font-bold text-[#2E4358]">{winRate}</p>
              </div>
            )}
            {maxDrawdown && (
              <div className="rounded-2xl bg-[#F7FAFC]/80 px-3 py-2.5">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-[#5A7188]">Max Drawdown</p>
                <p className="mt-0.5 text-lg font-bold text-[#7A2F2F]">{maxDrawdown}</p>
              </div>
            )}
            {sharpe && (
              <div className="rounded-2xl bg-[#F7FAFC]/80 px-3 py-2.5">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-[#5A7188]">Sharpe Ratio</p>
                <p className="mt-0.5 text-lg font-bold text-[#2E4358]">{sharpe}</p>
              </div>
            )}
          </div>
        )}
        ...
        {/* Monthly bar chart */}
        {monthlyReturns && (
          <div className="mb-4 rounded-2xl bg-[#F7FAFC]/80 px-3 py-2.5">
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-[#5A7188]">Monthly Returns</p>
            <MonthlyBarChart data={monthlyReturns} />
            <div className="mt-1 flex justify-between">
              {monthlyReturns.map((d, i) => (
                <span key={i} className="text-[8px] text-[#9FB4C1]">{d.month}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {/* Risk + min investment */}
        {(riskLevel || minInvestment) && (
          <div className="mb-4 flex items-center justify-between rounded-2xl bg-[#F7FAFC]/80 px-3 py-2.5">
            {riskLevel && (
              <div>
                <p className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-[#5A7188]">Risk Level</p>
                <RiskMeter level={riskLevel} />
              </div>
            )}
            {minInvestment && (
              <div className="text-right">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-[#5A7188]">Min. Investment</p>
                <p className="mt-0.5 text-base font-bold text-[#0F1A28]">{minInvestment}</p>
              </div>
            )}
          </div>
        )}
        ...
        <button onClick={() => setExpanded(!expanded)}
          className="mb-3 flex w-full items-center justify-between rounded-2xl bg-[#F7FAFC]/80 px-4 py-3 text-sm text-[#2E4358] transition hover:bg-[#EDF4F8]">
          <span className="font-medium text-[#0F1A28]">Fund Details</span>
          <span className={`text-[#9FB4C1] transition-transform duration-200 ${expanded ? "rotate-180" : ""}`}>▼</span>
        </button>

        {expanded && (
          <div className="mb-4 grid gap-2 text-sm leading-6 text-[#2E4358]">
...
function DashMetric({ label, value, sub, trend, color = "text-[#0F1A28]" }) {
  return (
    <div className="rounded-3xl border border-white/60 bg-white/75 p-5 shadow-[0_8px_30px_rgba(15,26,40,0.06)] backdrop-blur-md transition hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(15,26,40,0.10)] sm:p-6">
      <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#5A7188]">{label}</p>
      <p className={`mt-2 text-3xl font-bold ${color}`}>{value}</p>
      {sub && <p className="mt-1 text-xs text-[#5A7188]">{sub}</p>}
      {trend !== undefined && (
        <div className={`mt-3 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${trend >= 0 ? "bg-[#DCEFE3] text-[#1F5E36]" : "bg-[#F3E4E4] text-[#7A2F2F]"}`}>
...
/* ─── Growth Curve Chart ─────────────────────────────────────────────── */
function GrowthChart({ data, width = 600, height = 200 }) {
  ...
  const pathD = visible.map((d, i) => {
    const x = toX(data.indexOf(d));
    const y = toY(d.value);
    return `${i === 0 ? "M" : "L"} ${x} ${y}`;
  }).join(" ");

  const fillD = visible.length > 1
    ? `${pathD} L ${toX(data.indexOf(visible[visible.length - 1]))} ${pad.t + ch} L ${toX(0)} ${pad.t + ch} Z`
    : "";
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" style={{ minWidth: 280 }}>
        <defs>
          <linearGradient id="growthFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#2E4358" stopOpacity="0.18" />
            <stop offset="100%" stopColor="#2E4358" stopOpacity="0.01" />
          </linearGradient>
        </defs>
        {/* Grid lines */}
        {yTicks.map((t, i) => (
          <g key={i}>
            <line x1={pad.l} y1={t.y} x2={pad.l + cw} y2={t.y} stroke="#C9D8E2" strokeWidth="1" strokeDasharray="4 4" />
            <text x={pad.l - 6} y={t.y + 4} textAnchor="end" fontSize="9" fill="#9FB4C1">{t.label}</text>
          </g>
        ))}
        {/* Fill */}
        {fillD && <path d={fillD} fill="url(#growthFill)" />}
        {/* Line */}
        {pathD && <path d={pathD} fill="none" stroke="#0F1A28" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />}
        {/* X labels */}
        {xLabels.map((d, i) => {
          const idx = data.indexOf(d);
          return <text key={i} x={toX(idx)} y={height - 6} textAnchor="middle" fontSize="9" fill="#9FB4C1">{d.label}</text>;
        })}
        {/* Current value dot */}
        {visible.length > 1 && (
          <circle
            cx={toX(data.indexOf(visible[visible.length - 1]))}
            cy={toY(visible[visible.length - 1].value)}
            r="4" fill="#0F1A28" />
        )}
      </svg>
    </div>
...
function WinLossDonut({ winRate }) {
  const r = 40, cx = 56, cy = 56, stroke = 10;
  ...
  return (
    <div className="flex items-center gap-4">
      <svg width="112" height="112" viewBox="0 0 112 112">
        ...
        <circle cx={cx} cy={cy} r={r} fill="none" stroke="#1F5E36" strokeWidth={stroke}
          strokeDasharray={`${winDash} ${circ}`} strokeLinecap="round"
          transform={`rotate(-90 ${cx} ${cy})`}
          style={{ transition: "stroke-dasharray 1.5s ease-out" }} />
        <text x={cx} y={cy - 4} textAnchor="middle" fontSize="14" fontWeight="bold" fill="#0F1A28">{winRate}%</text>
        <text x={cx} y={cy + 12} textAnchor="middle" fontSize="8" fill="#9FB4C1">WIN RATE</text>
      </svg>
      <div className="space-y-2">
        <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-[#1F5E36]" /><span className="text-xs text-[#2E4358]">Wins: {winRate}%</span></div>
        <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-[#E6EEF2] border border-[#C9D8E2]" /><span className="text-xs text-[#2E4358]">Losses: {100 - winRate}%</span></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-3xl border border-white/60 bg-white/75 p-6 shadow-[0_12px_40px_rgba(15,26,40,0.06)] backdrop-blur-md transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_24px_70px_rgba(15,26,40,0.12)] sm:p-8">
      ...
      <div className="mt-6 space-y-3">
        {points.map((p, i) => <div key={i} className="rounded-2xl bg-[#F7FAFC]/80 px-4 py-3 text-sm text-[#2E4358]">{p}</div>)}
      </div>
  ...
  return (
    <div className="rounded-3xl border border-white/60 bg-white/75 p-6 shadow-[0_12px_40px_rgba(15,26,40,0.05)] backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_55px_rgba(15,26,40,0.10)]">
      ...
      <div className="mt-5 space-y-3">
        {bullets.map((b, i) => <div key={i} className="rounded-2xl bg-[#F7FAFC]/80 px-4 py-3 text-sm text-[#2E4358]">{b}</div>)}
      </div>
    </div>
  );
}
...
function AnimatedStatCard({ label, value, suffix = "", decimals = 0, started }) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-white/50 bg-white/55 p-5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:bg-white/70 hover:shadow-[0_12px_30px_rgba(15,26,40,0.10)]">
      ...
      <p className="mt-2 text-2xl font-bold text-[#0F1A28] tabular-nums">{display}{suffix}</p>
    </div>
  );
}
...
═══════════════════════════════════════════════════════════════════════ */
const GROWTH_DATA = [
  { label: "Jan", value: 0 }, { label: "Feb", value: 2.1 }, { label: "Mar", value: 5.8 },
  { label: "Apr", value: 4.2 }, { label: "May", value: 9.4 }, { label: "Jun", value: 12.1 },
  { label: "Jul", value: 10.8 }, { label: "Aug", value: 15.3 }, { label: "Sep", value: 18.7 },
  { label: "Oct", value: 22.4 }, { label: "Nov", value: 26.1 }, { label: "Dec", value: 31.2 },
];

const FUND_DATA = [
  {
    label: "Fund 1", name: "KaizenCapitalGroup.Xau-TMGM", focus: "Gold CFD", status: "Live",
    strategy: "Gold Scalping & Intra-day", managers: "1", brokerage: "TMGM",
    totalReturn: "+18.9%", winRate: "79.4%", maxDrawdown: "-4.2%", sharpe: "2.14",
    riskLevel: "Medium", minInvestment: "$500",
    sparkData: [12, 14, 13, 16, 15, 18, 17, 19, 21, 20, 22, 24, 23, 26],
    monthlyReturns: [
      { month: "J", val: 2.1 }, { month: "F", val: 1.8 }, { month: "M", val: 3.2 },
      { month: "A", val: -0.4 }, { month: "M", val: 2.8 }, { month: "J", val: 1.9 },
      { month: "J", val: 3.1 }, { month: "A", val: 2.4 },
    ],
    primaryLink: "https://signal.tmc2lnbmfs.com/portal/registration/subscription/94720/KCG-TMGM",
    secondaryLinks: [{ label: "Ratings", url: "https://ratings.tmgmplatform.com/widgets/shared/5173e304d7494051b27287f70426a327?lang=en%3Fpreview%3DP3U9ODIxMzA2JmE9MTM0NjMmcD0xMzgzNCZ3PTEmcz01MTczZTMwNGQ3NDk0MDUxYjI3Mjg3ZjcwNDI2YTMyNw%3D%3D" }],
  },
  {
    label: "Fund 1a", name: "KaizenCapitalGroup.Xau-MB", focus: "Gold CFD", status: "Live",
    strategy: "Gold Scalping & Intra-day", managers: "1", brokerage: "MultiBank",
    totalReturn: "+22.1%", winRate: "71.6%", maxDrawdown: "-5.8%", sharpe: "1.87",
    riskLevel: "Medium", minInvestment: "$1,500",
    sparkData: [10, 12, 11, 15, 14, 17, 16, 20, 19, 22, 21, 25, 24, 27],
    monthlyReturns: [
      { month: "J", val: 3.1 }, { month: "F", val: 2.4 }, { month: "M", val: -0.8 },
      { month: "A", val: 4.2 }, { month: "M", val: 1.6 }, { month: "J", val: 3.8 },
      { month: "J", val: 2.1 }, { month: "A", val: 3.4 },
    ],
  ...
  { label: "Fund 6", name: "Phoenix", focus: "Forex mixed assets", strategy: "Automated trading mix of all instruments (exact strategy to be explained)", managers: "Potential fully automated managed fund (1)", brokerage: "MultiBank", status: "N/A" },
  { label: "Fund 7", name: "Forex fotune AI", focus: "EURUSD", strategy: "Automated trading mix of EUR instruments (exact strategy to be explained)", managers: "1", brokerage: "MultiBank", status: "N/A" },
  {
    label: "Fund 8", name: "The Alpha Fund", focus: "Gold trading", status: "Live",
    strategy: "Manual trading", managers: "2 traders", brokerage: "TMGM",
    totalReturn: "+24.7%", winRate: "73.2%", maxDrawdown: "-6.1%", sharpe: "1.96",
    riskLevel: "Medium", minInvestment: "$1,000",
    sparkData: [8, 11, 13, 12, 16, 15, 18, 20, 19, 23, 22, 26, 25, 29],
    monthlyReturns: [
      { month: "J", val: 4.1 }, { month: "F", val: -1.2 }, { month: "M", val: 3.8 },
      { month: "A", val: 2.9 }, { month: "M", val: 1.4 }, { month: "J", val: 4.2 },
      { month: "J", val: 3.6 }, { month: "A", val: 2.8 },
    ],
    primaryLink: "https://signal.tmc2lnbmfs.com/portal/registration/subscription/67622/Alpha",
    secondaryLinks: [{ label: "Ratings", url: "https://ratings.tmgmplatform.com/widgets/shared/05a7391d205e4c82982ea3141e98aee5?lang=en?preview=P3U9N2M1Y2IwJmE9MTg5ODgmcD0xOTUwMCZ3PTEmcz0wNWE3MzkxZDIwNWU0YzgyOTgyZWEzMTQxZTk4YWVlNQ==" }],
  },
  { label: "Fund 9", name: "Algo Amalgamation Fund", focus: "Multi-asset (Gold, Forex, Crypto & others covered in Funds 1–8)", strategy: "Fully algorithmic trading — amalgamation of strategies from Funds 1–8 into one unified system", managers: "Mixture of algorithmic bots (no human managers)", brokerage: "MultiBank / TradeSmart / TMGM", status: "N/A" },
  ...
  {
    label: "Fund 11", name: "MAMALYN Fund", focus: "EUR/USD", status: "Live",
    strategy: "Fully algorithmic trading", managers: "1", brokerage: "MultiBank",
    totalReturn: "+31.5%", winRate: "68.9%", maxDrawdown: "-7.4%", sharpe: "1.74",
    riskLevel: "High", minInvestment: "$3,000",
    sparkData: [6, 9, 11, 14, 13, 17, 16, 20, 22, 21, 25, 28, 27, 31],
    monthlyReturns: [
      { month: "J", val: 5.2 }, { month: "F", val: 3.1 }, { month: "M", val: -2.1 },
      { month: "A", val: 6.4 }, { month: "M", val: 4.8 }, { month: "J", val: -1.2 },
      { month: "J", val: 5.9 }, { month: "A", val: 4.1 },
    ],
    primaryLink: "https://social.multibankfx.com/portal/registration/subscription/89236/mamalynMin3000dollars",
    secondaryLinks: [{ label: "FX Blue", url: "https://www.fxblue.com/users/mamalyn" }, { label: "Myfxbook", url: "https://www.myfxbook.com/members/Panevino83/mamalyn-mt4-31229860/11078849" }],
  },
  {
    label: "Fund 12", name: "CXFund", focus: "Gold trading", status: "Disconnected",
    strategy: "Manual trading", managers: "2 traders", brokerage: "TMGM",
    primaryLink: "https://signal.tmc2lnbmfs.com/portal/registration/subscription/69413/CXFund2026",
    secondaryLinks: [{ label: "Ratings", url: "https://ratings.tmgmplatform.com/widgets/shared/cc306ad97ef243a5aa092cd4d0d226bb?lang=en?preview=P3U9NjJiODU0JmE9MTg3MzkmcD0xOTI0NyZ3PTEmcz1jYzMwNmFkOTdlZjI0M2E1YWEwOTJjZDRkMGQyMjZiYg==" }],
  },
];
...
export default function Home() {
  ...
  const navLinks = [
    { href: "#home", label: "Home" }, { href: "#overview", label: "Overview" },
    { href: "#market-data", label: "Market Data" }, { href: "#funds", label: "Funds" },
    { href: "#performance", label: "Performance" }, { href: "#investor-funnel", label: "Investors" },
    { href: "#social-proof", label: "Proof" }, { href: "#activity", label: "Activity" },
    { href: "#why-kcg", label: "Why KCG" }, { href: "#contact-form", label: "Contact" },
  ];

  return (
    <>
      <style>{`
        @keyframes floatUp { 0% { transform: translateY(0px) scale(1); } 100% { transform: translateY(-30px) scale(1.08); } }
      ...
      <main className="min-h-screen bg-[#E6EEF2] text-[#0F1A28] overflow-x-hidden">
          ...
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6 sm:py-4">
            <div className="flex items-center gap-3 min-w-0">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#0F1A28] sm:h-8 sm:w-8">
                <span className="text-[9px] font-black tracking-tight text-white sm:text-[10px]">KCG</span>
              </div>
            ...
            <div className="flex items-center gap-2">
              <a href="#contact-form" className="hidden shrink-0 rounded-full bg-[#0F1A28] px-4 py-2 text-xs font-semibold text-white transition hover:scale-105 hover:bg-[#1A2A3D] sm:inline-block sm:px-5 sm:text-sm">Get Started</a>
              <button onClick={() => setMenuOpen(!menuOpen)} className="flex flex-col justify-center gap-[5px] p-2 md:hidden" aria-label="Toggle menu">
          ...
          {menuOpen && (
            <div className="animate-slide-down border-t border-white/30 bg-[#E6EEF2]/96 backdrop-blur-xl md:hidden">
              <div className="mx-auto max-w-6xl px-4 py-4 space-y-1 sm:px-6">
                {navLinks.map((l) => (
                  <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className="block rounded-xl px-4 py-3 text-sm font-medium text-[#2E4358] transition hover:bg-white/60 hover:text-[#0F1A28]">{l.label}</a>
                ))}
                <div className="pt-2">
                  <a href="#contact-form" onClick={() => setMenuOpen(false)} className="block rounded-full bg-[#0F1A28] px-5 py-3 text-center text-sm font-semibold text-white">Get Started</a>
                </div>
              </div>
            </div>
          )}
        ...
        <section id="home" className="relative min-h-screen overflow-hidden px-4 pb-16 pt-24 sm:px-6 sm:pt-28">
          ...
          <div className="relative z-10 mx-auto max-w-6xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#9FB4C1]/50 bg-white/50 px-4 py-1.5 backdrop-blur-sm sm:mb-8" style={{ animation: heroVisible ? "fadeUp 0.6s ease-out 0.1s both" : "none" }}>
            ...
            <div style={{ animation: heroVisible ? "fadeUp 0.7s ease-out 0.2s both" : "none" }}>
              <h1 className="max-w-5xl text-5xl font-bold leading-[1.08] tracking-tight text-[#0F1A28] sm:text-6xl md:text-7xl lg:text-8xl">
                ...
                strategy for <span className="relative inline-block">long-term
                  <svg className="absolute -bottom-1 left-0 w-full" viewBox="0 0 300 8" fill="none"><path d="M2 6 C60 2, 120 2, 180 4 C240 6, 280 4, 298 3" stroke="#9FB4C1" strokeWidth="2.5" strokeLinecap="round" fill="none" /></svg>
                </span> growth.
              </h1>
            </div>
            <p className="mt-6 max-w-2xl text-base leading-7 text-[#2E4358] sm:mt-8 sm:text-lg sm:leading-8" style={{ animation: heroVisible ? "fadeUp 0.7s ease-out 0.35s both" : "none" }}>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:mt-10 sm:flex-row sm:gap-4" style={{ animation: heroVisible ? "fadeUp 0.7s ease-out 0.45s both" : "none" }}>
              <a href="#contact-form" className="group relative overflow-hidden rounded-full bg-[#0F1A28] px-8 py-4 text-center font-semibold text-white transition-all hover:scale-105 hover:shadow-[0_12px_30px_rgba(15,26,40,0.25)]">
              ...
              <a href="#funds" className="rounded-full border border-[#2E4358]/40 bg-white/50 px-8 py-4 text-center font-semibold text-[#0F1A28] backdrop-blur-sm transition-all hover:scale-105 hover:bg-white/75 hover:border-[#2E4358]/70">View Funds →</a>
            </div>
            ...
            <div className="mt-8 overflow-hidden rounded-2xl border border-white/50 bg-white/50 py-3 backdrop-blur-sm" style={{ animation: heroVisible ? "fadeUp 0.7s ease-out 0.65s both" : "none" }}>
              <div className="ticker-track flex gap-0">
                {[...Array(2)].map((_, si) => (
                  <div key={si} className="flex shrink-0 items-center px-4">
        ...
        <FadeInSection id="market-data" className="px-4 py-20 sm:px-6 sm:py-24">
          <div className="mx-auto max-w-6xl">
            ...
            <p className="mb-10 max-w-3xl text-lg text-[#2E4358]">Live market widgets for gold, EUR/USD, Bitcoin, DXY, and oil.</p>
            <div className="mb-6">
        ...
        <FadeInSection id="funds" className="bg-[#F3F7FA] px-4 py-20 sm:px-6 sm:py-24">
          <div className="mx-auto max-w-6xl">
            ...
            <p className="mb-12 max-w-3xl text-lg text-[#2E4358]">Explore KCG's live offerings and developing fund structures. Live funds include sparklines, monthly returns, risk profiles, and direct access links.</p>
        ...
        {/* ══════════════════════════════════════════════
            PERFORMANCE DASHBOARD — Phase 3 NEW SECTION
        ══════════════════════════════════════════════ */}
        <FadeInSection id="performance" className="px-4 py-20 sm:px-6 sm:py-24">
          <div className="mx-auto max-w-6xl">
            <p className="mb-4 text-sm font-semibold uppercase tracking-[0.2em] text-[#5A7188]">Performance Dashboard</p>
            <h2 className="mb-4 text-4xl font-bold text-[#0F1A28] md:text-5xl">Data-driven execution across all KCG systems.</h2>
            <p className="mb-12 max-w-3xl text-lg text-[#2E4358]">A consolidated view of KCG's live fund performance metrics, growth trajectory, risk-adjusted returns, and system activity.</p>
            ...
            <div className="mb-8 grid gap-6 lg:grid-cols-3">
              <div className="lg:col-span-2 rounded-3xl border border-white/60 bg-white/75 p-6 shadow-[0_12px_40px_rgba(15,26,40,0.06)] backdrop-blur-md sm:p-8">
                <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#5A7188]">Cumulative Growth Curve</p>
                    <p className="mt-1 text-2xl font-bold text-[#0F1A28]">+31.2% YTD</p>
                  </div>
                  <div className="flex gap-2">
                    {["1M", "3M", "6M", "YTD", "1Y"].map((t) => (
                      <button key={t} className={`rounded-full px-3 py-1 text-xs font-semibold transition ${t === "YTD" ? "bg-[#0F1A28] text-white" : "bg-[#F7FAFC] text-[#5A7188] hover:bg-[#EDF4F8]"}`}>{t}</button>
                    ))}
                  </div>
                </div>
              ...
              <div className="rounded-3xl border border-white/60 bg-white/75 p-6 shadow-[0_12px_40px_rgba(15,26,40,0.06)] backdrop-blur-md sm:p-8">
                ...
                <div className="mt-6 space-y-3">
                  {[{ label: "Total Trades", value: "8,658" }, { label: "Winning Trades", value: "6,325" }, { label: "Losing Trades", value: "2,333" }, { label: "Avg Trade Duration", value: "4.2h" }].map((s) => (
                    <div key={s.label} className="flex items-center justify-between rounded-2xl bg-[#F7FAFC]/80 px-4 py-2.5">
            ...
            <div className="mb-8 rounded-3xl border border-white/60 bg-white/75 p-6 shadow-[0_12px_40px_rgba(15,26,40,0.06)] backdrop-blur-md sm:p-8">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#E6EEF2]">
                      {["Fund", "Status", "Total Return", "Win Rate", "Max DD", "Sharpe", "Risk"].map((h) => (
                        <th key={h} className="pb-3 text-left text-[10px] font-bold uppercase tracking-[0.15em] text-[#5A7188]">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {FUND_DATA.filter(f => f.totalReturn).map((f) => (
                      <tr key={f.label} className="border-b border-[#F3F7FA] transition hover:bg-[#F7FAFC]/60">
                        <td className="py-3 font-semibold text-[#0F1A28]">{f.name.length > 24 ? f.name.slice(0, 24) + "…" : f.name}</td>
                        <td className="py-3">
                          <span className="inline-flex items-center gap-1 rounded-full bg-[#DCEFE3] px-2 py-0.5 text-[10px] font-bold text-[#1F5E36]">
                            <span className="h-1 w-1 rounded-full bg-[#1F5E36] animate-pulse" /> Live
                          </span>
                        </td>
                        <td className="py-3 font-bold text-[#1F5E36]">{f.totalReturn}</td>
                        <td className="py-3 text-[#2E4358]">{f.winRate}</td>
                        <td className="py-3 text-[#7A2F2F]">{f.maxDrawdown}</td>
                        <td className="py-3 text-[#2E4358]">{f.sharpe}</td>
                        <td className="py-3"><RiskMeter level={f.riskLevel} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { label: "Systems Online", value: "4", icon: "⚡", sub: "Live & executing" },
                { label: "Algorithms Active", value: "7", icon: "🤖", sub: "Across all platforms" },
                { label: "Brokerages Connected", value: "3", icon: "🏦", sub: "TMGM · MultiBank · TradeSmart" },
                { label: "Instruments Traded", value: "5", icon: "📈", sub: "Gold · FX · Crypto · CFDs" },
              ].map((s) => (
                <div key={s.label} className="rounded-2xl border border-white/60 bg-white/75 p-5 shadow-sm backdrop-blur-md transition hover:-translate-y-1">
        ...
        {/* ── INVESTOR FUNNEL ── */}
        <FadeInSection id="investor-funnel" className="bg-[#F3F7FA] px-4 py-20 sm:px-6 sm:py-24">
          <div className="mx-auto max-w-6xl">
        ...
        {/* ── SOCIAL PROOF ── */}
        <FadeInSection id="social-proof" className="px-4 py-20 sm:px-6 sm:py-24">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-16">
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {["Broker / Partner Logo", "Platform Logo", "Media / Network Logo", "Strategic Partner Logo"].map(n => (
                  <div key={n} className="flex h-20 items-center justify-center rounded-2xl border border-white/50 bg-white/65 px-6 text-center text-sm font-semibold text-[#5A7188] shadow-sm backdrop-blur-md">{n}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        {/* ── ACTIVITY ── */}
        <FadeInSection id="activity" className="bg-[#F3F7FA] px-4 py-20 sm:px-6 sm:py-24">
          <div className="mx-auto max-w-6xl">
                ...
                <div key={i} className="rounded-2xl border border-white/40 bg-white/70 p-5 shadow-sm backdrop-blur-md transition hover:-translate-y-0.5 hover:shadow-md">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="text-right sm:text-right">
                      <p className="text-sm font-bold text-[#1F5E36]">{trade.profit}</p>
                      <p className="text-xs text-[#5A7188]">{trade.result}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-2">
            <div>
              ...
              <div className="mt-8 space-y-4">
                {[{ href: "mailto:cottrell@kaizencapitalgrp.com", label: "Email", value: "cottrell@kaizencapitalgrp.com" }, { href: "https://t.me/YOUR_USERNAME", label: "Telegram", value: "@YOUR_USERNAME" }, { href: "https://calendly.com/YOUR_LINK", label: "Book a Call", value: "Schedule an investor consultation" }].map((c) => (
                  <a key={c.label} href={c.href} target={c.href.startsWith("mailto") ? undefined : "_blank"} rel="noopener noreferrer" className="block rounded-2xl border border-white/40 bg-white/60 p-5 shadow-sm backdrop-blur-md transition hover:-translate-y-1 hover:shadow-md">
                    <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#5A7188]">{c.label}</p>
            ...
            <div className="rounded-[28px] border border-white/30 bg-white/70 p-6 shadow-[0_20px_60px_rgba(15,26,40,0.08)] backdrop-blur-md sm:rounded-[32px] sm:p-8">
              {submitted && <div className="mb-5 rounded-2xl border border-[#C9D8E2] bg-[#EDF4F8] px-4 py-3 text-sm text-[#0F1A28]">Your investor inquiry draft was opened successfully.</div>}
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button type="submit" className="w-full rounded-full bg-[#0F1A28] px-6 py-4 font-semibold text-white transition hover:scale-[1.02] hover:bg-[#1A2A3D] hover:shadow-[0_8px_24px_rgba(15,26,40,0.20)]">Submit Investor Inquiry</button>
              </form>
            </div>
          </div>
        </FadeInSection>
