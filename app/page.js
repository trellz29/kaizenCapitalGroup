    ...
    <>
      ...
      <div style={{ opacity: showLoader ? 0 : 1, transition: "opacity 0.8s ease 0.2s", visibility: showLoader ? "hidden" : "visible" }}>
        ...
        {/* ??????????????????????????????????????
            SCENE 1 — HERO
        ?????????????????????????????????????? */}
        <section id="home" style={{ position: "relative", minHeight: "100vh", overflow: "hidden", clipPath: "none" }}>
          <CinematicHero />
          <HeroGlobe />
        </section>
          ...
          <div className="marquee-track">
            {[...Array(3)].map((_, si) => (
              <div key={si} style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
                {["INSTITUTIONAL EXECUTION", "MULTIPLE FUNDS", "DISCIPLINED CAPITAL", "$847M+ VOLUME", "9.2% AVG MONTHLY RETURN", "COPY TRADING LIVE", "KAIZEN CAPITAL GROUP", "VERIFIED STRATEGIES"].map((item, i) => (
        ...
        <Scene id="overview" style={{ background: "#070F1E", padding: "80px 0" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 clamp(1.5rem,5vw,3rem)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(260px,100%),1fr))", gap: 16 }}>
              {[
                { title: "Institutional Execution", body: "KCG deploys capital through verified brokerages with institutional-grade execution infrastructure across multiple active funds." },
